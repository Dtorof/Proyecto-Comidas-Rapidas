new Vue({   
  ...
  methods: {
    ...
    addQuantityToProducts(){
      if (this.allProducts?.burgers){
        this.allProducts.burgers = this.allProducts.burgers.map(prod =>({...prod, qty: 1}))
      }
      if (this.allProducts?.hot_dogs){
        this.allProducts.hot_dogs = this.allProducts.hot_dogs.map(prod => ({...prod, qty: 1}))
      }
    },
